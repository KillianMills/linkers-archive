...
async def handler(uri="wss://example/chat"):
	async with aiosqlite.connect('links.db') as db:
		await db.execute('''CREATE TABLE IF NOT EXISTS links (id INTEGER PRIMARY KEY, url TEXT NOT NULL UNIQUE, first_seen TIMESTAMP DEFAULT CURRENT_TIMESTAMP, source TEXT)''')
		await db.commit()
	async with websockets.connect(uri) as ws:
		async for message in ws:
			...
			for u in set(URL_RE.findall(text)):
				...
				n = normalise_url(u)
				async with aiosqlite.connect('links.db') as db:
					await db.execute('INSERT OR IGNORE INTO links (url, source) VALUES (?,?)', (n, 'websocket'))
					await db.commit()
